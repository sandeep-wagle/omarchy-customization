...
Item {
  ...
  // Mirrors the on-disk `bar-auto-hide` flag (also under the state toggles
  // directory). Presence selects AUTO_HIDE, absence selects ALWAYS_VISIBLE.
  // This flag file is the single source of truth for the mode; the bar never
  // reads or mutates `bar.mode` inside shell.json. Updated by the probe +
  // FileView + syncMode IPC below, mirroring the `bar-off` pipeline exactly.
  property bool barAutoHide: false
  readonly property bool autoHideMode: barAutoHide && !barHidden

  // Auto-hide runtime state. Event-driven only: these flip in response to
  // pointer enter/leave events and one-shot timers, never from polling.
  property bool autoHideReveal: true
  // One counter shared by every bar surface and activation zone so crossing
  // between them cannot strand a stale hovered flag, like barHoverCount.
  property int autoHideHoverCount: 0
  readonly property bool autoHideHovered: autoHideHoverCount > 0
  // Behaviour knobs, read from the `bar:` subtree of shell.json at config
  // reload (autoHideDelay, autoHideRevealDelay, activationZone) with sane
  // defaults so an unconfigured fallback config still behaves.
  property int autoHideDelay: 600
  property int autoHideRevealDelay: 60
  property real autoHideActivationHeight: 6
  ...
  function applyBarConfig() {
    ...
    if (typeof config.autoHideDelay === "number" && config.autoHideDelay > 0)
      root.autoHideDelay = Math.round(config.autoHideDelay)
    if (typeof config.autoHideRevealDelay === "number" && config.autoHideRevealDelay >= 0)
      root.autoHideRevealDelay = Math.round(config.autoHideRevealDelay)
    if (typeof config.activationZone === "number" && config.activationZone > 0)
      root.autoHideActivationHeight = Math.max(2, config.activationZone)
  ...
  function setBarHovered(hovered) {
    ...
    // Auto-hide: hovering the bar keeps it revealed, leaving starts the
    // collapse countdown. This only feeds the reveal/hide state machine; an
    // enter/leave event can never change the mode itself.
    root.setAutoHideHovered(hovered)
  }

  function setAutoHideHovered(hovered) {
    if (!root.autoHideMode) return
    autoHideHoverCount = Math.max(0, autoHideHoverCount + (hovered ? 1 : -1))
    if (root.autoHideHovered) {
      autoHideRevealTimer.stop()
      autoHideTimer.stop()
      root.autoHideReveal = true
    } else {
      autoHideRevealTimer.stop()
  ...
  // Auto-hide state machine, driven entirely by pointer enter/leave events
  // (setAutoHideHovered) and these two single-shot timers. Nothing polls.
  Timer {
    id: autoHideTimer
    interval: root.autoHideDelay
    repeat: false
    // Hide after the pointer leaves; the bar stays while it is hovered, an
    // activation zone is hovered, or a widget popout is open.
    onTriggered: {
      if (root.autoHideMode && !root.autoHideHovered && !root.barHovered && !root.activePopout)
        root.autoHideReveal = false
    }
  }

  Timer {
    id: autoHideRevealTimer
    interval: root.autoHideRevealDelay
    repeat: false
    onTriggered: {
      if (root.autoHideMode && root.autoHideHovered)
        root.autoHideReveal = true
    }
  }

  // Entering AUTO_HIDE starts with the bar visible, then collapses it once
  // the countdown elapses; leaving AUTO_HIDE forces it visible.
  onAutoHideModeChanged: {
    autoHideRevealTimer.stop()
    if (root.autoHideMode) {
      root.autoHideReveal = true
      autoHideTimer.restart()
    } else {
      autoHideTimer.stop()
  ...
  // Presence of the `bar-off` flag = bar hidden; presence of `bar-auto-hide`
  // = auto-hide mode. Watching the parent toggles directory because FileView
  // can't observe a file that doesn't exist yet; the flags are created/removed
  // by `omarchy-toggle-bar` and `omarchy-toggle-bar-mode`.
  Process {
    id: barHiddenProbe
    running: true
  ...
  Process {
    id: barAutoHideProbe
    running: true
    command: ["bash", "-c", "[[ -f $HOME/.local/state/omarchy/toggles/bar-auto-hide ]] && echo yes || echo no"]
    stdout: SplitParser { onRead: function(line) { root.barAutoHide = String(line).trim() === "yes" } }
  }
  FileView {
    path: root.home + "/.local/state/omarchy/toggles"
    watchChanges: true
    printErrors: false
    onFileChanged: { barHiddenProbe.running = true; barAutoHideProbe.running = true }
  }

  // The directory watch can permanently stop delivering events after flag
  // changes land in quick succession (and is globally disabled when
  // QS_DISABLE_FILE_WATCHER=1), stranding the bar in the wrong state until the
  // shell restarts. `omarchy-toggle-bar` / `omarchy-toggle-bar-mode` nudge the
  // matching handler here after flipping a flag so the probe re-reads it even
  // when the watch has gone quiet.
  IpcHandler {
    target: "omarchy.bar"
  ...
  component BarPanel: PanelWindow {
    ...
    // Collapsed (auto-hide or `bar-off`) parks the bar just past the screen
    // edge, keeping the surface alive; showing is only a margin change. The
    // Behaviour animates that margin so reveals slide instead of jumping.
    readonly property real parkedMargin: root.barHidden || (root.autoHideMode && !root.autoHideReveal) ? -root.barSize : 0
    property real effectiveMargin: parkedMargin
    Behavior on effectiveMargin {
      NumberAnimation { duration: 220; easing.type: Easing.OutCubic }
    }

    margins {
      top: root.position === "top" ? effectiveMargin : 0
      bottom: root.position === "bottom" ? effectiveMargin : 0
      left: root.position === "left" ? effectiveMargin : 0
      right: root.position === "right" ? effectiveMargin : 0
    }
  ...
  // Activation zone for AUTO_HIDE: a thin, transparent, input-only strip at
  // the screen edge. It is not a bar — no content, ExclusionMode.Ignore, tiny —
  // it only exists so the collapsed (fully parked) bar can still detect the
  // top-edge hover gesture that reveals it. One instance per monitor.
  component AutoHideActivationPanel: PanelWindow {
    id: activationPanel

    // Present in AUTO_HIDE only; unmap in ALWAYS_VISIBLE so the edge stays
    // fully usable for windows.
    visible: root.autoHideMode
    color: "transparent"
    surfaceFormat.opaque: false
    exclusionMode: ExclusionMode.Ignore
    WlrLayershell.namespace: "omarchy-bar-activation"
    WlrLayershell.layer: WlrLayer.Top
    ...
    implicitWidth: root.vertical ? Math.max(2, Math.ceil(root.autoHideActivationHeight)) : 0
    implicitHeight: root.vertical ? 0 : Math.max(2, Math.ceil(root.autoHideActivationHeight))
    ...
    MouseArea {
      ...
      onEntered: root.setAutoHideHovered(true)
      onExited: root.setAutoHideHovered(false)
    }
  }
