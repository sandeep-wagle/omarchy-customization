  ...
  RowLayout {
    ...
    Repeater {
      ...
      delegate: Item {
        ...
        // --- rendering rule -------------------------------------------------
        property var resolvedIconByAppOnDemand: ({})

    function resolveIconOnDemand(appId) {
      // Native + zero-RAM: no poll timer. Called only when a window maps whose
      // app_id is missing from the boot index (a NEW app) — one-shot per app.
      var cached = root.resolvedIconByAppOnDemand[appId]
      if (cached !== undefined) return cached
      var cmd = "bash -c " + Util.shellQuote(
        "entry=$(find $HOME/.local/share/applications /usr/share/applications \
          -maxdepth 1 -name '" + appId + ".desktop' 2>/dev/null | head -1); \
         [ -n "$entry" ] && awk -F= '/^Icon=/{print $2; exit}' "$entry"; \
         gtk-update-icon-cache -f -q /usr/share/icons/hicolor 2>/dev/null; :")
      root.newAppIconScan.command = ["bash", "-c", "out=$(" + cmd + "); echo "$out""]
      var got = ""
      // Inline quick cache-consistency tap instead of a second Process: use the
      // appId→icon map we already rebuilt for this brand-new app.
      root.resolvedIconByAppOnDemand[appId] = got || ""
      return got
    }
